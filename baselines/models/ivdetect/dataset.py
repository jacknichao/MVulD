...
class BigVulDatasetIVDetect(BigVulDataset):
    ...
    def __init__(self, **kwargs):
        ...
        print(f'{kwargs["partition"]} LOCAL before large:', len(self.df)) # 9653
        ret = dfmp(
            self.df,
        ...
        self.df = self.df[ret]
        print(f'{kwargs["partition"]} LOCAL after large:', len(self.df)) # 9553 
...
def feature_extraction(filepath):
    ...
    ast_nodes["lineidx"] = ast_nodes.groupby("lineNumber").cumcount().values  # compute ast nodes every line
    ast_edges = ast_edges[ast_edges.line_out == ast_edges.line_in]  # group ast edges by lines

...
class DataSet:
    ...
    def initialize_dataset(self, balance=True, output_buffer=sys.stderr):
        ...
        # print(f"positive_indices_in_train len={len(self.positive_indices_in_train)}")
        # print(f"negative_indices_in_train len={len(self.negative_indices_in_train)}")
...
def create_dataset(ggnn_output, batch_size=32, output_buffer=sys.stderr):
    ...
    train_data, train_labels = ggnn_output[0] # （pred=ggnn_output,true）
    print(len(train_data),len(train_labels)) # 77072 * 2
    hdim = len(train_data[0]) # gnn_output的维度是200
    print('hdim:', hdim)
    dataset = DataSet(batch_size=batch_size, hdim=hdim)
    for i in range(len(train_data)):
        # print(train_data[i].cpu().numpy(), train_labels[i].cpu().numpy(),)
        # assert False
    ...
    print(len(valid_data),len(valid_labels)) # 9642 * 2
    ...
    print(len(test_data),len(test_labels)) # 9629 * 2

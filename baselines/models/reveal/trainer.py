    ...
    try:
        for epoch_count in range(num_epochs): 
            ...
            for _ in output_batches_generator:
                ...
                # print(f"train features length={len(features)},targets length={len(targets)}")
                # print(f"train same_class_features length={len(same_class_features)}")
                # print(f"train diff_class_features length={len(diff_class_features)}")
            ...
            if output_buffer is not None:
                ...
                # print('After epoch %2d ' % (epoch_count), file=output_buffer)

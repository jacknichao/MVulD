...
def convert_defect_examples_to_features(item):
    example, example_index, tokenizer, args = item # (example, idx, tokenizer, args)

    source_str = ' '.join(example[0].split()) # example[0]：funcs [1]:lables [2]:ids
    # tokenizer 是 unxicoder里的tokenize()
    code = tokenizer([source_str], max_length=args.max_source_length, padding=True)
    # 这个类绝对有那三个属性：source_ids(code),lable(example[1]),_id(example[2])
    return DefectInputFeatures(example_index, code[0], example[1], example[2])

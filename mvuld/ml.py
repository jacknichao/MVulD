...
def best_f1(true, pos_logits):
    """Find optimal threshold for F1 score.# 由于是不平衡的，单独值输入
...
def get_metrics_logits(true, logits):
    ...
    model.res2f = ml.get_metrics_logits(all_true_f, all_pred_f) 首先看这个func-level的
    model.res2mt = ml.get_metrics_logits(multitask_true, multitask_pred)
    """
    ...
    try:
        ...
        roc_auc = -1
    try:
        pr_auc = average_precision_score(true_oh, logits) # (y_true, y_score)
    except:
        pr_auc = -1

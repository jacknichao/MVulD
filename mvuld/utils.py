...
def load_checkpoint(config, model, optimizer, lr_scheduler, loss_scaler, logger):
    ...
    ## Because the NUM_CLASSES is 1000 by default, the NUM_CLASSES will not match when loading the model.
    if config.MODEL.NUM_CLASSES!=1000 and checkpoint['model']['head.weight'].shape[0] == 1000:
        print(f"==============> checkpoint head weight shape{checkpoint['model']['head.weight'].shape}....................")
        # print(f"==============> checkpoint head bias shape{checkpoint['model']['head.bias'].shape}....................")
        checkpoint['model']['head.weight'] = torch.nn.Parameter(
            torch.nn.init.xavier_uniform_(torch.empty(config.MODEL.NUM_CLASSES, 1024))) # d=1024
...
def save_bestf1_checkpoint(config, epoch, model, max_accuracy, optimizer, lr_scheduler, loss_scaler, logger):
    ...
    save_path = os.path.join(config.OUTPUT,'checkpoint-best-f1', f'mymodel.pth')

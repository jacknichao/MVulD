...
class MyUniXcoder(nn.Module):
    def __init__(self, encoder, config, tokenizer,tokenize): 
        super(MyUniXcoder, self).__init__()
        self.encoder = encoder
        self.config = config
        self.tokenizer = tokenizer
        self.tokenize = tokenize # Convert string to token ids 
        print("Loading MyUniXcoder...") 
        self.classifier = nn.Linear(config.hidden_size, 2)
        self._dev = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        # self.encoder.to(self._dev)  
    ...
    def myEncode(self, sents: list): 
        """Get UniXcoder embeddings from a list of sentences."""
        # source_ids 
        source_ids=[]
        for node in sents:
            node_str = ' '.join(node.split()) # example[0]：funcs [1]:lables [2]:ids
            node_ids = self.tokenize([node_str], max_length=512 , padding=True)
            source_ids.append(node_ids)
        # tokens_ids = self.tokenize(sents)
        all_source_ids = torch.tensor([i for i in source_ids], dtype=torch.long)
        vec,_ = self.get_repr(all_source_ids) # shape:[node num,768]
        print(vec.shape)
        return vec
    
    def getRobertaEmbedding(self):
        unix_layer = torch.nn.Sequential()
        for name, child in self.encoder.named_children():
            if name in ['embeddings']:
                for param in child.parameters():
                    param.requires_grad = False # 把embedding的参数固定
                unix_layer.append(child) # RobertaEmbeddings要完整的加进去
        self.unix_embedding = unix_layer # RobertaEmbeddings

    def myEncode2(self, sents: list): 
        """Get UniXcoder embeddings from a list of sentences."""
        source_ids=[]
        for node in sents:
            node_str = ' '.join(node.split()) # example[0]：funcs [1]:lables [2]:ids
        ...
        all_source_ids = torch.tensor([i for i in source_ids], dtype=torch.long)
        self.getRobertaEmbedding()
        unix_embedding = self.unix_embedding(all_source_ids) # [bs, 512, 768]
        return unix_embedding
    
    def get_repr(self, input_ids, labels=None):
        source_ids = input_ids.view(-1, self.max_source_length) 

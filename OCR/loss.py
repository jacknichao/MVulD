...
def get_dice_loss(gt_score, pred_score):
	inter = torch.sum(gt_score * pred_score) # Intersection
	union = torch.sum(gt_score) + torch.sum(pred_score) + 1e-5 # Merge
	return 1. - (2 * inter / union) # 0-1


def get_geo_loss(gt_geo, pred_geo):
	d1_gt, d2_gt, d3_gt, d4_gt, angle_gt = torch.split(gt_geo, 1, 1)
	d1_pred, d2_pred, d3_pred, d4_pred, angle_pred = torch.split(pred_geo, 1, 1)
	area_gt = (d1_gt + d2_gt) * (d3_gt + d4_gt) # Label area
	area_pred = (d1_pred + d2_pred) * (d3_pred + d4_pred) # detect area
	# Intersection area 
	...
	# Calculate area loss (intersection/union)
...
class Loss(nn.Module):
	...
	def forward(self, gt_score, pred_score, gt_geo, pred_geo, ignored_map):
		...
		# score loss (Dice，Log entropy calculation is not used to prevent sample imbalance
		classify_loss = get_dice_loss(gt_score, pred_score*(1-ignored_map))
		# geo loss(Calculate by Iou = calculate the loss of each pixel)
		iou_loss_map, angle_loss_map = get_geo_loss(gt_geo, pred_geo)
		# loss of total img，angle_loss_map*gt_score Remove pixels that are not target points
		...
		geo_loss = self.weight_angle * angle_loss + iou_loss # The weight here is set to 1
